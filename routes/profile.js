...
router.get("/profile/:prof_id", function(req, res){
    User.findById(req.params.prof_id, function(err, foundProf){
        if(err){
            ...
            res.render("profile/show", {foundProf: foundProf});
        }
    })
});
...
router.get("/profile/:prof_id/edit", function(req, res){
    User.findById(req.params.prof_id, function(err, foundProf){
        if(err){
            ...
            res.render("profile/edit", {foundProf: foundProf});
        }
    })
});

router.put("/profile/:prof_id", function(req, res){
    User.findByIdAndUpdate(req.params.prof_id,  req.body.user, function(err, updated){
        if(err){
            console.log(err);
        } else {
...
router.get("/profile/:prof_id/editPass", function(req, res){
    User.findById(req.params.prof_id, function(err, foundProf){
        if(err){
            ...
            res.render("profile/editPass", {foundProf: foundProf});
        }
    })
});
